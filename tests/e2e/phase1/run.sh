...
#
# Builds FSharp.PureAnalyzer, runs it on tests/e2e/customer-fixture/Program.fs via the
...
ANALYZER_OUT="$ROOT/tests/e2e/.artifacts/analyzer-drop"
REPORT_DIR="$ROOT/tests/e2e/.artifacts/phase1"
...
dotnet build "$ROOT/tests/e2e/phase1/AssertDefinitionBadges/AssertDefinitionBadges.fsproj" -c Release --nologo -v q

if [[ "${UPDATE_BASELINE:-0}" == "1" ]]; then
  echo "==> Phase 1: UPDATE_BASELINE=1 — rewriting $BASELINE from SARIF"
  dotnet run --project "$ROOT/tests/e2e/phase1/AssertDefinitionBadges/AssertDefinitionBadges.fsproj" -c Release --no-build -- \
    --sarif "$SARIF_PATH" \
