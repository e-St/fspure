...
let private writeBaseline (path: string) (actual: Map<string, string>) =
    ...
    writer.WriteString(
        "$schema_comment",
        "Baseline definition badges for tests/e2e/customer-fixture/Program.fs. PURE002 → impure, PURE003 → pure. Regenerate with: UPDATE_BASELINE=1 bash tests/e2e/phase1/run.sh"
    )
