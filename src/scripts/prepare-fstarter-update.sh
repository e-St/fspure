...
#
# Usage:
#   bash src/scripts/prepare-fstarter-update.sh /path/to/fstarter [analyzer-version]
#
# Writes:
...
PACK="$ROOT/src/scripts/integrations/fstarter"
...
if [[ -f "$OVERLAY/Directory.Build.props" ]]; then
  cp -f "$OVERLAY/Directory.Build.props" "$FSTARTER/Directory.Build.props"
fi

# Version pins consumed by setup-fspure.sh
SKILL_REF="${FSPURE_SKILL_REF:-}"
if [[ -z "$SKILL_REF" && -f "$VERSIONS_FILE" ]]; then
  # shellcheck disable=SC1090
  source "$VERSIONS_FILE"
  SKILL_REF="${FSPURE_SKILL_REF:-main}"
fi
SKILL_REF="${SKILL_REF:-main}"
CLI_RELEASE="${FSPURE_CLI_RELEASE:-}"
if [[ -z "$CLI_RELEASE" && -f "$VERSIONS_FILE" ]]; then
  # shellcheck disable=SC1090
  source "$VERSIONS_FILE"
  CLI_RELEASE="${FSPURE_CLI_RELEASE:-fspure-latest}"
fi
CLI_RELEASE="${CLI_RELEASE:-fspure-latest}"
{
  echo "# Generated by e-St/fspure prepare-fstarter-update.sh — do not edit by hand in fstarter."
  echo "# Source of truth: e-St/fspure src/scripts/integrations/fstarter/versions.env"
...
# Sync metadata for humans / CI
{
  ...
  echo "source_path=src/scripts/integrations/fstarter"
