#!/usr/bin/env bash
# Publish official packages from src/docs/releases/manifest.json pending block.
...
m = json.loads(pathlib.Path("src/docs/releases/manifest.json").read_text())
if not m.get("pending"):
    print("ERROR: src/docs/releases/manifest.json has no pending release", file=sys.stderr)
    sys.exit(1)
...
PY
  ...
  local vsix="src/editor/vscode-extension/fsharp-pure-decorations-${ver}.vsix"
...
python3 - <<'PY'
import json, pathlib
path = pathlib.Path("src/docs/releases/manifest.json")
...
bash "$ROOT/src/scripts/release/apply-version-pins.sh"
