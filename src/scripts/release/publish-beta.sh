...
BASE_ANALYZER="$(python3 -c "import json; print(json.load(open('src/docs/releases/manifest.json'))['lastOfficial']['FSharp.PureAnalyzer'])")"
BASE_COLLECTOR="$(python3 -c "import json; print(json.load(open('src/docs/releases/manifest.json'))['lastOfficial']['fspure-collector'])")"
...
dotnet nuget push "src/FSharp.PureAnalyzer/nupkgs/"*.nupkg \
...
dotnet nuget push "src/fspure-collector/nupkgs/"*.nupkg \
  --api-key "$TOKEN" --source "$SRC" --skip-duplicate
