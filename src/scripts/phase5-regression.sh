...
#
# Uses only:
#   - src/tests/e2e/customer-fixture          → foundational-only badges
#   - src/samples/fspure-ready-lib      → library embed (PackageReference + ProjectReference)
...
#   bash src/scripts/phase5-regression.sh
#
# Env:
...
SAMPLE="$ROOT/src/samples/fspure-ready-lib"
...
[[ -f "$ROOT/src/scripts/fspure-ready-lib-gate.sh" ]] || die "monorepo gate script missing"
chmod +x \
  "$ROOT/src/scripts/fspure-ready-lib-gate.sh" \
  "$ROOT/src/scripts/phase5-regression.sh" \
  "$SAMPLE/src/scripts/"*.sh \
  "$ROOT/src/tests/e2e/phase1/run.sh" \
  2>/dev/null || true
...
# ---------------------------------------------------------------------------
bash "$ROOT/src/scripts/fspure-ready-lib-gate.sh"
...
bash "$SAMPLE/src/scripts/assert-golden-pure-methods.sh" "$PURE_JSON"
...
root = Path("src/samples/fspure-ready-lib")
...
if [[ "${SKIP_UNIT:-0}" == "1" ]]; then
  ...
  dotnet test "$ROOT/src/tests/FSharp.PureSchema.Tests/FSharp.PureSchema.Tests.fsproj" \
    -c "$CONFIGURATION" \
    --verbosity minimal \
    --filter "FullyQualifiedName~ResourceReaderTests"

  # Analyser fallback + composition + override contracts
  dotnet test "$ROOT/src/tests/FSharp.PureAnalyzer.Tests/FSharp.PureAnalyzer.Tests.fsproj" \
    -c "$CONFIGURATION" \
    --verbosity minimal \
...
echo "   foundational:     src/tests/e2e/phase1"
echo "   PackageReference: src/scripts/fspure-ready-lib-gate.sh + golden"
echo "   ProjectReference: src/samples/fspure-ready-lib Consumer"
