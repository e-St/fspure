...
**Script:** `bash src/tests/e2e/phase1/run.sh`

| Step | Detail |
|------|--------|
| Build | `FSharp.PureAnalyzer` → `src/tests/e2e/.artifacts/analyzer-drop/` |
| Analyze | `fsharp-analyzers` on `src/tests/e2e/customer-fixture/Program.fs` |
| Assert | SARIF `PURE002`/`PURE003` vs baseline `src/tests/e2e/customer-fixture/expectations.json` |
...
```bash
UPDATE_BASELINE=1 bash src/tests/e2e/phase1/run.sh
# review git diff of expectations.json, then commit
```

Artifacts: `src/tests/e2e/.artifacts/phase1/`

## Phase 2 — Visual VS Code decorations

**Script:** `bash src/tests/e2e/phase2/run.sh`  
**Dev container:** `src/tests/e2e/phase2/.devcontainer/` (e2e-only; daily Codespaces use root `.devcontainer/` — “fspure IDE”)
...
| Capture | Playwright screenshots → `src/tests/e2e/.artifacts/phase2/*.png` |
...
devcontainer up --workspace-folder . --config src/tests/e2e/phase2/.devcontainer/devcontainer.json
devcontainer exec --workspace-folder . --config src/tests/e2e/phase2/.devcontainer/devcontainer.json \
  bash src/tests/e2e/phase2/run.sh
```

Or run `src/tests/e2e/phase2/run.sh` on any host that has .NET 10, Node 20, code-server, and Playwright deps.

## Library embed (Phase 4)

**Script:** `bash src/tests/e2e/ready-lib/run.sh` (same as `bash src/scripts/fspure-ready-lib-gate.sh`)

Local NuGet feed only: pack monorepo analyzer → pack `src/samples/fspure-ready-lib` → consumer + hard PURE002/PURE003 asserts. See [ready-lib/README.md](ready-lib/README.md). CI: `.github/workflows/fspure-ready-lib-gate.yml`.

## Phase 5 permanent regression

**Script:** `bash src/scripts/phase5-regression.sh`  
...
| Slice | How |
|-------|-----|
| Foundational only | `src/tests/e2e/phase1` + `customer-fixture` |
...
| Golden pure methods | `src/samples/fspure-ready-lib/tests/golden/` |
