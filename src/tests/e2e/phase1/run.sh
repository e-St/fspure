...
#
# Builds FSharp.PureAnalyzer, runs it on src/tests/e2e/customer-fixture/Program.fs via the
...
FIXTURE_DIR="$ROOT/src/tests/e2e/customer-fixture"
ANALYZER_OUT="$ROOT/src/tests/e2e/.artifacts/analyzer-drop"
REPORT_DIR="$ROOT/src/tests/e2e/.artifacts/phase1"
...
dotnet build "$ROOT/src/tests/e2e/phase1/AssertDefinitionBadges/AssertDefinitionBadges.fsproj" -c Release --nologo -v q

if [[ "${UPDATE_BASELINE:-0}" == "1" ]]; then
  echo "==> Phase 1: UPDATE_BASELINE=1 — rewriting $BASELINE from SARIF"
  dotnet run --project "$ROOT/src/tests/e2e/phase1/AssertDefinitionBadges/AssertDefinitionBadges.fsproj" -c Release --no-build -- \
    --sarif "$SARIF_PATH" \
