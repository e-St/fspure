...
ARTIFACTS="$ROOT/src/tests/e2e/.artifacts/phase2"
SHOT_PROJ="$ROOT/src/tests/e2e/phase2/ScreenshotCapture/ScreenshotCapture.fsproj"
...
bash "$ROOT/src/tests/e2e/phase2/prepare-workspace.sh"

echo "======== Phase 2: start VS Code (code-server) ========"
bash "$ROOT/src/tests/e2e/phase2/start-code-server.sh"
...
if [[ "${SKIP_INSTALL_PLAYWRIGHT:-0}" != "1" ]]; then
  ...
  PW_PS1="$(find "$ROOT/src/tests/e2e/phase2/ScreenshotCapture/bin" -name 'playwright.ps1' 2>/dev/null | head -1 || true)"
