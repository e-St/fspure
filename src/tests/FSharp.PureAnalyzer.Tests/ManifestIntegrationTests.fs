...
module private Env =
    let pureLibDll () =
        Path.Combine(repoRoot (), "src", "tests", "FSharp.PureAnalyzer.Tests",
            "integration",
            "PureLib",
            "bin",
            "Release",
    ...
    let ensurePureLibBuilt () =
        let proj =
            Path.Combine(repoRoot (), "src", "tests", "FSharp.PureAnalyzer.Tests", "integration", "PureLib", "PureLib.fsproj")
...
let ``assembly with zero pure.json resources falls back to foundational only`` () =
    ...
    let fixtureProj =
        Path.Combine(root, "src", "tests", "fixtures", "ZeroPureResources", "ZeroPureResources.fsproj")
...
let ``CLI analyser emits PURE003 for library-backed pure wrapper (ProjectReference)`` () =
    ...
    let consumer =
        Path.Combine(root, "src", "tests", "FSharp.PureAnalyzer.Tests",
            "integration",
            "ConsumerProjectRef",
