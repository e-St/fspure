...
module private Paths =
    let fixtures () =
        Path.Combine(repoRoot (), "src", "tests", "FSharp.PureAnalyzer.Tests", "msbuild-fixtures")
...
let ``FSharp.PureAnalyzer nupkg ships build targets and fspure-collector tools`` () =
    let root = repoRoot ()
    let analyzerProj = Path.Combine(root, "src", "FSharp.PureAnalyzer", "FSharp.PureAnalyzer.fsproj")
    let outDir =
