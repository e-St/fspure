...
```text
e-St/fspure   →   src/samples/fspure-ready-lib/
```
...
1. Copies `src/samples/fspure-ready-lib/` into a clone of the satellite (rsync; excludes `bin`/`obj`/`artifacts`).
...
1. Edit **`src/samples/fspure-ready-lib/`** in **fspure**.
